<template>
  <div class="relative">
    <input
      ...
      class="h-12 w-48 border-2 px-2.5 pr-10 text-xs text-main_color focus:outline-none active:outline-none xs:w-64 sm:w-48 md:text-sm lg:w-80 lg:text-base xl:w-96"
      type="text"
      :placeholder="placeholderValue"
      :title="titleValue"
    />

    <div class="absolute right-4 top-3 sm:hidden" ref="dropdown">
      ...
      <div v-if="setSearchIsOpen" class="absolute right-[-65%] top-[150%] z-10 w-40">
        <button
          @click="[changeSearchByVar('paintings'), toggleSetSearch()]"
          class="block w-full bg-main_color px-1 py-2 text-left text-xs text-white"
        >
          По названию картины
        </button>
        <button
    ...
    <div class="absolute top-3 hidden sm:right-1 sm:block">
...
const placeholderValue = computed(() =>
  searchBy.value === 'paintings' ? 'Название картины' : 'Имя автора'
)
const titleValue = computed(() =>
  searchBy.value === 'paintings' ? 'Введите название картины' : 'Введите имя автора'
)
...
const closeDropdown = (event: MouseEvent) => {
  if (dropdown.value && !dropdown.value.contains(event.target as Node))
    setSearchIsOpen.value = false
}

onMounted(() => {
  // следим за кликами. Если выпадающий список выбора поиска "по автору" или по "названию картины" открыт и был произведён клик в другую область экрана, то выпадающий список будет закрыт
